...
def test_all_themes_work(generator, sample_code):
    ...
    for theme in AVAILABLE_THEMES:
        try:
            ...
            assert (
                theme_found
            ), f"Theme name not found in response for: {theme}. Response: {response_text}"
    ...
    assert (
        success_rate >= 0.9
    ), f"Too many themes failed: {len(failed_themes)}/{len(AVAILABLE_THEMES)}. Failed themes: {[t[0] for t in failed_themes]}"
...
def test_dark_vs_light_themes(generator, sample_code):
    ...
    assert (
        dark_img_base64 != light_img_base64
    ), "Dark and light themes should produce different images"
...
def test_theme_consistency(generator, sample_code):
    ...
    assert (
        response1 == response2
    ), "Same theme with same parameters should produce identical responses"
...
""",
    ...
    for theme in themes_to_test:
        for language, code in test_codes.items():
            ...
            assert (
                img_base64
            ), f"Failed to generate image for theme {theme} with language {language}"
            assert (
                f"**Language**: {language.title()}" in response_text
            ), f"Language not detected correctly for {language}"
...
def test_edge_case_themes(generator, sample_code):
    ...
    for theme in edge_case_themes:
        if theme in AVAILABLE_THEMES:
            try:
                response_text, img_base64 = generator.generate(
                    code=sample_code, theme=theme, frame_style="minimal", background="transparent"
                )
