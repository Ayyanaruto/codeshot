...
def apply_rounded_corners(code_img: Image.Image, radius: int = 12) -> Image.Image:
    ...
    mask_draw.rounded_rectangle(
        [0, 0, mask_size[0], mask_size[1]], radius=radius * mask_scale, fill=255
    )
...
def add_watermark(
    frame_img: Image.Image, watermark_text: str, font, frame_width: int, frame_height: int
) -> Image.Image:
    ...
    watermark_draw.rounded_rectangle([bg_x1, bg_y1, bg_x2, bg_y2], radius=6, fill=(0, 0, 0, 100))

    # Draw watermark text
    watermark_draw.text(
        (watermark_x, watermark_y), watermark_text, fill=(255, 255, 255, 200), font=font
    )
