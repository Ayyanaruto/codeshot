...
from config.constants import QUALITY_SCALE, THEME_MAPPINGS
...
def adjust_line_spacing(code: str, font_size: int) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        is_dense_line = (
            len(line.strip()) > 60  # Long line
            or sum(1 for c in line if c in "{}[]().,;:=+-*/<>!&|") > 8  # Many symbols
            or line.strip().startswith(
                ("def ", "class ", "function ", "if ", "for ", "while ")
            )  # Key statements
        )
        ...
        should_add_spacing = (
            consecutive_long_lines >= 2  # Multiple consecutive dense lines
            and i < len(lines) - 1  # Not the last line
            and lines[i + 1].strip()  # Next line is not empty
            and not lines[i + 1].strip().startswith((" ", "\t"))  # Next line is not indented
            and len(lines[i + 1].strip()) > 20  # Next line is substantial
        )

        # Also add spacing before function/class definitions for better separation
        if (
            i < len(lines) - 1
            and lines[i + 1].strip().startswith(("def ", "class ", "function ", "@"))
            and line.strip()
            and not line.strip().startswith(("def ", "class ", "function ", "@"))
        ):
...
def create_code_image(
    ...
    if code_length > 20000 or line_count > 200:
        ...
        logger.info(
            f"Using low quality rendering (1x) and reduced font size ({font_size}px) for large code: {code_length} chars, {line_count} lines"
        )
    elif code_length > 10000 or line_count > 100:
        quality_scale = 2  # Medium quality for medium code
        font_size = max(10, font_size - 2)
        logger.info(
            f"Using medium quality rendering (2x) and reduced font size ({font_size}px) for medium code: {code_length} chars, {line_count} lines"
        )
    else:
    ...
    if quality_scale > 1:
        ...
        code_img = code_img_high_res.resize((target_width, target_height), Image.Resampling.LANCZOS)
