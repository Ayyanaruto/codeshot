...
from src.core.renderer import detect_language, create_code_image
from src.utils.validation import (
    ValidationResult, validate_theme, validate_frame_style, validate_background,
    validate_font_family, validate_font_size, validate_boolean_with_random,
    validate_string_optional, format_response_text
)
...
from src.effects.special import apply_border_glow, apply_rounded_corners, add_watermark
from src.frames.macos import draw_macos_frame
from src.frames.windows import draw_windows_frame
from config.constants import PADDING, TITLE_BAR_HEIGHT, EXTRA_SPACE
...
class CodeshotGenerator:
    ...
    def generate(
        ...
        with log_performance(self.logger, "screenshot generation"):
            ...
            rounded_corners = validate_boolean_with_random(rounded_corners, "Rounded Corners", True, self.validation_result)
            border_glow = validate_boolean_with_random(border_glow, "Border Glow", False, self.validation_result, 0.2)
            
            window_title = validate_string_optional(window_title, "Window Title")
            watermark = validate_string_optional(watermark, "Watermark")
            
            self.logger.info(f"Final parameters: theme={theme}, frame={frame_style}, background={background}, font={font_family}, size={font_size}")
            ...
            if frame_style == "macos":
                ...
                draw_macos_frame(draw, frame_width, title_bar_height, window_title, language, font_family)
            elif frame_style == "windows":
                self.logger.debug("Drawing Windows window frame")
                draw_windows_frame(draw, frame_width, title_bar_height, window_title, language, font_family)
            
            # Apply shadow effect
            if shadow and frame_style != "none":
                self.logger.debug("Applying shadow effect")
                frame_img = apply_shadow_effect(frame_img, code_img, code_x, code_y, rounded_corners)
            
            # Paste code image
            self.logger.debug("Pasting code image onto frame")
            frame_img.paste(code_img, (int(code_x), int(code_y)), code_img if rounded_corners else None)
            ...
            if watermark:
                ...
                frame_img = add_watermark(frame_img, watermark, watermark_font, frame_width, frame_height)
            ...
            effects = []
            if shadow: effects.append("Shadow")
            if reflection: effects.append("Reflection")  
            if border_glow: effects.append("Glow")
            if rounded_corners: effects.append("Rounded")
            
            response_text = format_response_text(
                language, theme, frame_style, background, font_family, 
                font_size, frame_width, frame_height, effects, self.validation_result
            )
