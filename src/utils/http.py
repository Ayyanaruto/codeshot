...
async def fetch_code_from_url(url: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(
                f"Successfully fetched {content_length} characters, {line_count} lines from {url}"
            )
            ...
            if needs_truncation:
                if content_length > MAX_CODE_SIZE:
                    logger.warning(
                        f"Code content is too large ({content_length} chars), truncating to {MAX_CODE_SIZE} chars"
                    )
                if line_count > MAX_LINES:
                    logger.warning(
                        f"Code has too many lines ({line_count}), truncating to {MAX_LINES} lines"
                    )
                ...
                logger.info(
                    f"Content truncated to {len(truncated_content)} characters, {final_line_count} lines"
                )
